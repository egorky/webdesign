            ...
            <main className="relative z-10">
                <section className="max-w-7xl mx-auto px-6 lg:px-12 pt-24 pb-16">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                        <div>
                            <span className="inline-block bg-purple-400 text-white px-4 py-1.5 border-2 border-black font-black uppercase tracking-widest text-[10px] mb-6">
                                {t('solutions.omnichannel.title')}
                            </span>
                            <h1 className="text-5xl md:text-7xl font-black tracking-tighter mb-8 leading-[0.9] uppercase">
                                {t('solutions.omnichannel.hero')}
                            </h1>
                            <p className="text-xl text-darkBlue/70 font-bold leading-relaxed mb-10 max-w-xl">
                                {t('solutions.omnichannel.details')}
                            </p>
                            <Link to="/contact" className="b-solid bg-purple-400 text-white px-10 py-4 font-black uppercase text-sm tracking-widest hover:brightness-110 transition-all inline-block shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
                                {t('nav.getStarted')}
                            </Link>
                        </div>
                ...
                <section className="max-w-7xl mx-auto px-6 lg:px-12 py-24 border-t-2 border-black/10">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {t('solutions.omnichannel.features', { returnObjects: true }).map((feature, i) => (
                            <div key={i} className="card-border bg-white p-8 hover:-translate-y-2 transition-transform">
                                <span className="material-symbols-outlined text-purple-500 mb-6">join_inner</span>

        ...
        <div className="bg-background-light min-h-screen">
            <header className="relative overflow-hidden pt-24 pb-20 hero-pattern border-b-2 border-black">
                <div className="max-w-7xl mx-auto px-6">
                    <div className="inline-block px-4 py-2 border-2 border-black bg-orange-300 font-black text-xs tracking-widest uppercase mb-8 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                        {t('industries.retail.benefitLabel')}: {t('industries.retail.benefitValue')}
                    </div>
                    <h1 className="font-display text-5xl lg:text-7xl font-black leading-tight mb-8 tracking-tighter text-darkBlue">
                        {t('industryPages.retail.hero.title')}
                    </h1>
                    <p className="text-xl text-darkBlue/80 max-w-3xl leading-relaxed font-medium">
                        {t('industryPages.retail.hero.subtitle')}
                    </p>
                </div>
            </header>
            <main className="max-w-7xl mx-auto px-6 py-24">
                <div className="mb-20">
                    <h2 className="font-display text-4xl lg:text-5xl font-black mb-6 tracking-tight text-darkBlue uppercase">{t('industryPages.retail.useCasesTitle')}</h2>
                        ...
                        <div key={index} className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                            ...
                            <div className={index % 2 === 1 ? 'lg:order-1' : ''}>
                                {useCase.image ? (
                                    ...
                                    <div className="bg-orange-50 b-solid p-12 aspect-video flex items-center justify-center shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]"><span className="material-symbols-outlined text-9xl text-orange-600/30">shopping_cart</span></div>
                                )}
                            </div>
                        </div>
                    ))}
